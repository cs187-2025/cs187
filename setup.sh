...
echo "✅ Only the 'otter-latest' environment is created/updated"
echo ""
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    echo "     • Create otter-latest environment (or update if exists)"
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    echo "  [DRY RUN] Expected environment: otter-latest"
...
EOF
    ...
    if conda install -n otter-latest --dry-run numpy >/dev/null 2>&1; then
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    if command_exists pyenv; then
        ...
        echo "     Run: conda activate otter-latest && jupyter lab"
    else
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    echo "=================="
    echo ""
    echo "✅ otter-latest environment ready"
...
echo "      conda activate otter-latest"
echo ""
...
echo "   conda env remove -n otter-latest"
echo ""
...
if [ "$CONDA_DEFAULT_ENV" = "otter-latest" ]; then
    echo "✅ You're ready to go! The otter-latest environment is active."
else
    echo "⚠️  Run 'conda activate otter-latest' to start working."
fi
